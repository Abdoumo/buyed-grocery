import React from "react";
import { Svg, Path, G } from "react-native-svg";
import { useThemeHelper } from '../../helper';

const CartBagSuccessIcon = (props) => {
    const { colors } = useThemeHelper();
    const { width = 24, height = 24, color = colors.primary_color } = props
    return (
        <Svg width={width} height={height} viewBox="0 0 24 24" fill="none">
            <G id="SVGRepo_bgCarrier" stroke-width="0" />
            <G id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" />
            <G id="SVGRepo_iconCarrier">
                <Path d="M4.0828 10.8943C4.52171 8.55339 4.74117 7.38295 5.57434 6.69147C6.40752 6 7.59835 6 9.98003 6H14.0209C16.4026 6 17.5934 6 18.4266 6.69147C19.2598 7.38295 19.4792 8.55339 19.9181 10.8943L20.6681 14.8943C21.2853 18.186 21.5939 19.8318 20.6942 20.9159C19.7945 22 18.12 22 14.7709 22H9.23003C5.88097 22 4.20644 22 3.30672 20.9159C2.40701 19.8318 2.7156 18.186 3.3328 14.8943L4.0828 10.8943Z" fill={color} />
                <Path d="M9.75 5C9.75 3.75736 10.7574 2.75 12 2.75C13.2426 2.75 14.25 3.75736 14.25 5V6C14.25 5.99999 14.25 6.00001 14.25 6C14.816 6.00018 15.3119 6.00174 15.7499 6.01488C15.75 6.00993 15.75 6.00497 15.75 6V5C15.75 2.92893 14.0711 1.25 12 1.25C9.92893 1.25 8.25 2.92893 8.25 5V6C8.25 6.00498 8.25005 6.00995 8.25015 6.01491C8.68814 6.00175 9.18397 6.00021 9.75 6.00002C9.75 6.00002 9.75 6.00003 9.75 6.00002V5Z" fill={color} />
                <Path d="M15.5857 11.9685C15.8444 11.6451 15.792 11.1731 15.4685 10.9144C15.1451 10.6556 14.6731 10.7081 14.4143 11.0315L10.9375 15.3769L9.53033 13.9697C9.23744 13.6768 8.76256 13.6768 8.46967 13.9697C8.17678 14.2626 8.17678 14.7375 8.46967 15.0304L10.4697 17.0304C10.6205 17.1812 10.8285 17.2607 11.0415 17.2489C11.2545 17.2371 11.4524 17.1351 11.5857 16.9685L15.5857 11.9685Z" fill={colors.tea_green_color} />
            </G>
        </Svg>
    )
};

export default CartBagSuccessIcon;
